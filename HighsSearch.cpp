  ...
  if (mipsolver.mipdata_->upper_limit != kHighsInf) {
    ...
    if (lp->computeDualProof(mipsolver.mipdata_->domain,
      ...
      mipsolver.mipdata_->debugSolution.checkCut(inds.data(), vals.data(),
                                                 inds.size(), rhs);
      //std::lock_guard<std::recursive_mutex> guard(g_mutex);//→cutgeneration.cpp:generateConflict
...
HighsSearch::NodeResult HighsSearch::evaluateNode() {
  ...
  if (localdom.infeasible()) {
    ...
    //{
      //std::lock_guard<std::recursive_mutex> guard(g_mutex);
      localdom.conflictAnalysis(mipsolver.mipdata_->conflictPool);//HighsDomain.cpp:3处（已经最细）
    //}
  } else {
    lp->flushDomain(localdom);
    lp->setObjectiveLimit(mipsolver.mipdata_->upper_limit);
    ...
    if (localdom.infeasible()) {
        ...
        if (lp->unscaledDualFeasible(status)) {
          ...
          if (currnode.lower_bound > getCutoffBound()) {
            ...
            if (localdom.infeasible()) {
              ...
              {
                std::lock_guard<std::recursive_mutex> guard(g_mutex);//重复锁
                localdom.conflictAnalysis(mipsolver.mipdata_->conflictPool);
              }
      ...
      //{
        //std::lock_guard<std::recursive_mutex> guard(g_mutex);//待定(重复锁)
        addInfeasibleConflict();
      //}
      if (parent != nullptr && parent->lp_objective != -kHighsInf &&
          parent->branching_point != parent->branchingdecision.boundval) {
...
HighsSearch::NodeResult HighsSearch::branch() {
  ...
  while (nodestack.back().opensubtrees == 2 &&
    ...
    //{
      //std::lock_guard<std::recursive_mutex> guard(g_mutex);
      branchcand = selectBranchingCandidate(sbmaxiters, downNodeLb, upNodeLb);
    //}
    // if (!mipsolver.submip)
    //   printf("branching cand returned as %d\n", branchcand);
    NodeData& currnode = nodestack.back();
    childLb = currnode.lower_bound;
    //std::lock_guard<std::recursive_mutex> guard(g_mutex);//→1471
    if (branchcand != -1) {
      //{
        //std::lock_guard<std::recursive_mutex> guard(g_mutex);
      ...
      //}
        ...
        // std::lock_guard<std::recursive_mutex> guard(g_mutex);//→1466
        HighsInt col = branching.first;
      
      switch (childselrule) {
        //std::lock_guard<std::recursive_mutex> guard(g_mutex);//→1463
  ...
  //std::lock_guard<std::recursive_mutex> guard(g_mutex);//→1643
  inbranching = false;
  NodeData& currnode = nodestack.back();
  ...
  if (currnode.branchingdecision.column == -1) {
    //std::lock_guard<std::recursive_mutex> guard(g_mutex);//→1567
    double bestscore = -1.0;
  ...
  if (currnode.branchingdecision.column == -1) {
    //std::lock_guard<std::recursive_mutex> guard(g_mutex);// →1627
